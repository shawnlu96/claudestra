...
- **自动更新** — 在 Discord 里用自然语言说"升级一下"就触发 `git pull` + `pm2 restart ecosystem.config.cjs`（只重启 Claudestra 自己的 3 个进程，你 pm2 里跑的其他服务不受影响）。
...
bun src/manager.ts update    # git pull && pm2 restart ecosystem.config.cjs（只 Claudestra 自己 3 个进程）
```
