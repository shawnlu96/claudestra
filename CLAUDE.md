...
src/
  ...
  manager.ts manager/  CLI entry + switch; command families in manager/ (core = registry / output / flags; never import manager.ts from there)
  ...
  lib/                 pure / shared logic — the canonical helpers (see Anti-rot rule 4); repo paths only via lib/repo-root.ts
