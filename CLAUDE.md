...
2. **Loosening needs a paper trail.** Edit the baseline by hand *and* add `{key, from, to, why}` to its `raised[]` (why ≥ 10 characters); say it again in the commit message. The guard diffs against the base version's baseline (CI: `GUARD_BASE` = PR base / push `before`; locally: the fork point with upstream/`origin/main`), so an unrecorded raise fails CI even if it passed locally.
...
7. **No silent error swallowing.** Every `catch {}` / `.catch(() => {})` carries a sentence on why losing the error is harmless (placeholders like `/* ignore */`, `/* 同上 */`, `/* non-critical */` count as silent); log when you can.
