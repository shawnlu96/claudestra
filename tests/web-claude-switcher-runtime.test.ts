 ...
 * agent = CC）挂 CC 面板；Pi 挂 PiModelSwitcher；Codex 挂 CodexModelSwitcher；不认识的
 * 运行时不渲染——以前「第三种 runtime 继续走 CC 面板」，Codex 会话拿到 Claude 的模型下拉，
 * 点了恒报「回合进行中」。
 */
...
describe("switcherKindFor（顶栏挂哪种切换器）", () => {
  ...
  test("Codex 走自己的切换器", () => {
    expect(switcherKindFor("codex")).toBe("codex");
  });

  test("不认识的运行时不渲染（不落进 CC 面板）", () => {
